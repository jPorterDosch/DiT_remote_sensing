...
def eval_video_tracking_davis(
    ...
    for cnt in tqdm(range(1, len(frame_list))):
        ...
        frame_tar_avg, feat_tar, mask_neighborhood = label_propagation(
            args, model, frame_tar, used_frame_feats, used_segs, ori_h, ori_w, mask_neighborhood
        )
...
def label_propagation(
    args, model, frame_tar, list_frame_feats, list_segs, ori_h, ori_w, mask_neighborhood=None
):
    """
...
def extract_feature(args, model, frame, ori_h, ori_w, return_h_w=False):
    ...
    with torch.no_grad():
        dit_feat_raw, dit_ada = model.forward(
            args, frame, timestep=args.t, block_idx=args.k, ensemble_size=args.ensemble_size
        )  # c, h, w
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--k", nargs="+", type=int, default=[28], help="which dit block to extract the ft map"
    )  ###调参[0,57]
    parser.add_argument(
        "--img_size",
        type=int,
    ...
    parser.add_argument(
        "--ensemble_size", default=1, type=int, help="ensemble size for getting an image ft map"
    )
    parser.add_argument("--temperature", default=0.1, type=float, help="temperature for softmax")

    parser.add_argument(
        "--output_dir", type=str, help="Path where to save segmentations", default="./davis_results_flux/"
    )
    parser.add_argument(
        "--data_path",
        type=str,
