    ...
    for s in SEEDS:
        for tr, va in StratifiedKFold(5, shuffle=True, random_state=s).split(x[:, 0], y):
            ...
            for _ in range(EPOCHS):
                ...
                nn.functional.cross_entropy(m(xa), ya).backward(); opt.step()
            m.eval()
            ...
            deltas.append(delta); rms.append(pos_rms)
...
def direction_gate(x, y, seed=42):
    ...
    for s_ in SEEDS:
        for tr, va in StratifiedKFold(5, shuffle=True, random_state=s_).split(x[:, 0], y):
            ...
            st = whiten_fit(a); a, b = whiten(st, a), whiten(st, b)
            # forward + reversed copies of every sample
            a2 = np.concatenate([a, a[:, ::-1, :]]); ya = np.r_[np.zeros(len(a)), np.ones(len(a))]
            b2 = np.concatenate([b, b[:, ::-1, :]]); yb = np.r_[np.zeros(len(b)), np.ones(len(b))]
            ...
            for _ in range(EPOCHS):
                m.train(); opt.zero_grad()
                nn.functional.cross_entropy(m(xa), ya_t).backward(); opt.step()
            m.eval()
            with torch.no_grad():
...
if __name__ == "__main__":
    ...
    print(f"SUBSTRATE GATE (forward vs reversed on whitened features): {g.mean():.4f} "
          f"+- {g.std():.4f}  (chance 0.5)")
    if g.mean() < 0.7:
        print("  GATE FAIL: the encoder cannot read order on this substrate. The "
        ...
        print("  GATE PASS: the encoder CAN read order on whitened features, so a "
    ...
    if td.mean() < 1e-3:
        print("GATE FAIL: the traj encoder is permutation-INVARIANT -- traj and shuffle are "
    ...
    print(f"  traj - shuffle {diff.mean():+.4f}  [{lo:+.4f}, {hi:+.4f}]  "
