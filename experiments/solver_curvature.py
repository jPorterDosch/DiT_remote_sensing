...
def main() -> None:
    ...
    cfg = RunConfig(task="extract", model=ModelConfig(name="flux", ensemble_size=1),
                    ...
                    img_size=list(args.img_size), subset_size=args.subset_size,
                    subset_seed=42, batch_size=1, num_workers=4, label_fraction=1.0)
    seed_all(cfg.seed)
    ...
    with torch.no_grad():
        for n, batch in enumerate(loader):
            ...
            out = model._inner.invert_chain(
                img, cache_timesteps=T_GRID, num_inversion_steps=args.num_steps,
                block_idx=28, guidance=1.0, want_curvature=True, want_states=True)
    ...
    base = f"{args.dataset}_solvercurv_n{args.num_steps}"
    ...
    np.savez(os.path.join(args.out_dir, base + ".npz"),
             pred=P.astype(np.float32), pred_mid=M.astype(np.float32),
             states=S_.astype(np.float32), labels=y,
    ...
    meta = {"dataset": args.dataset, "img_size": list(args.img_size), "t": "non-terminal cached timesteps (terminal has no departing step)",
            "num_inversion_steps": args.num_steps, "subset_size": int(len(indices)),
            "subset_seed": 42, "guidance": 1.0, "k": 28,
