...
declare -A params
params=(
    [dataset_path]="/lustre/isaac24/scratch/jdosch1/DeepLearning/datasets/EuroSAT"
    [save_path]="$SAVE_DIR"
    [dit_model]="flux"
    [img_size]="224"
    [t]="260"
...
# This is where list-valued args belong (--label_fractions) since they can't
# go in the params array above.
#
...
parallel -j $PARALLEL_JOBS --delay 15 --shuf --verbose \
	python3 "$PROJECT_ROOT/eval_eurosat.py" \
        --cd \
        --label_fractions 1 5 10 50 100 \
        $SWEEP_PLACEHOLDERS \
    $SWEEP_VALUES
set +x
print_delim "## DONE"
