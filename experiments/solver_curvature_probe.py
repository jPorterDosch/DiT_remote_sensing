...
FINDINGS (2026-08-26/27, n=500). Curvature is class-informative alone (EuroSAT 0.65 vs raw
states 0.51). This grid lacked the decisive comparison (states+pred vs states+pred+curv);
the follow-up found curvature adds +0.067 beyond states AND velocity on EuroSAT, carried by
the spatial PATTERN, not magnitude. Superseded by curv5000_probe.py at n=5000.
"""
...
DATA = {
    "eurosat": (
        "models/solver_curvature/eurosat_solvercurv_n50.npz",
        "models/raw_xt/eurosat_invstate_n50_full.npz",
    ),
    "resisc45": (
        "models/solver_curvature/resisc45_solvercurv_n50.npz",
        ...
        for c in CS:
            ...
            for name, blk in blocks.items():
                for g, tag in [(1, "mean64"), (2, "2x2")]:
                    ...
                    print(
                        f"    {name:<9}{tag:<7} alone {alone.mean():.4f}   "
