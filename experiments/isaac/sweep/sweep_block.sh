...
#    cache is COMPLETE (npz + its _meta.json, which run.py writes last) is skipped, and a
#    run dir left by a killed extraction (no _meta.json: partial or missing npz) is removed
#    first, because run.py refuses an existing run dir -- so a re-queued task resumes.
#    Safe only because this k's dir is private to this task (splits run sequentially).
...
# cache killed mid-write never has one). A bare npz is treated as absent.
have_split() {
    local f
    for f in "$1"/*/multistep_"$2"_feats_oneshot_g1.0.npz; do
        [ -f "$f" ] && [ -f "${f%.npz}_meta.json" ] && return 0
    done
    return 1
}
# Remove run dirs under this task's PRIVATE sweep dir that hold no *_meta.json: each run
# dir is one split's extraction, so "no meta" means it was killed before finishing and
# run.py would refuse to reuse it ("Save directory already exists"). Never touches REUSE_DIR.
clear_incomplete() {
    local d
    for d in "$1"/*/; do
        [ -d "$d" ] || continue
        if ! ls "$d"*_meta.json >/dev/null 2>&1; then
            echo "k=$K: removing incomplete extraction $d (no _meta.json)"
            rm -rf "$d"
        fi
    done
}
...
if [ -n "$REUSE_K" ] && [ "$K" = "$REUSE_K" ] && [ -z "$SUBSET_ARGS" ]; then
    ...
    DIR="$SWEEP_ROOT/k$K"
    [ -d "$DIR" ] && clear_incomplete "$DIR"
    for split in "${SPLIT_LIST[@]}"; do
        SEED="$(seed_for "$split")"
        if have_split "$DIR" "$split"; then
            echo "k=$K $split: complete cache exists, skipping extraction"; continue
        fi
