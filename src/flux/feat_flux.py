...
class Featurizer:
    def __init__(self, name='flux-dev', null_prompt='', device='cuda'):
        ...
        print("Loading pre-computed null embeddings...")
        null_dict = torch.load("null_embeddings.pt", weights_only=True)
        self.prompt_embeds = null_dict["prompt_embeds"].to(device)
        self.text_ids = null_dict["text_ids"].to(device)
        self.vec = null_dict["vec"].to(device)
    ...
    def forward(self,
        ...
        prompt_embeds = self.prompt_embeds.expand(ensemble_size, -1, -1)
        text_ids = self.text_ids.expand(ensemble_size, -1, -1)
        vec = self.vec.expand(ensemble_size, -1)

        unet_ft_all = self.model(
...
class Featurizer4Eval(Featurizer):
    ...
    @torch.no_grad()
    def forward(self, args, img_tensor, caption="a photo of a image", category="image", timestep=261, block_idx=1, ensemble_size=1, guidance=3.5):
        ...
        model_output = self.model.forward_feat(
            img=img, img_ids=img_ids, txt=prompt_embeds, txt_ids=text_ids,
            y=vec, timesteps=t_vec, ft_indices=block_idx, cat=category, guidance=guidance_vec
        )
