    ...
    if args.dataset == "pascal":
        ...
        if args.AP10K_EVAL_SUBSET == "intra-species":
            categories = [
                ...
                for folder in os.listdir(os.path.join(data_dir, "ImageAnnotation", subfolder))
            ]
        elif args.AP10K_EVAL_SUBSET == "cross-species":
            categories = [
                subfolder
                for subfolder in subfolders
                if len(os.listdir(os.path.join(data_dir, "ImageAnnotation", subfolder))) > 1
            ]
...
def resize(img, target_res=224, resize=True, to_pil=True, edge=False, one_pading=False):
    ...
    if not edge:
        ...
        if original_height <= original_width:
            if resize:
                img = img.resize(
                    (target_res, int(np.around(target_res * original_height / original_width))),
            ...
            if resize:
                img = img.resize(
                    (int(np.around(target_res * original_width / original_height)), target_res),
                    Image.Resampling.LANCZOS,
                )
            width, height = img.size
            img = np.asarray(img)
...
def load_ap10k_data(path, size=768, category="cat", split="test", subsample=0):
    ...
    for pair in pairs:
        ...
        src_imname = (
            ((source_json_path).split(".")[0])
            .replace("ImageAnnotation", "JPEGImages")
            .replace("dataset/ap-10k/", "")
        )
        trg_imname = (
            ((target_json_path).split(".")[0])
            .replace("ImageAnnotation", "JPEGImages")
            .replace("dataset/ap-10k/", "")
        )
...
def load_spair_data(path, size=768, category="cat", split="test", subsample=None):
    ...
    for pair in pairs:
        ...
        if split == "test" or split == "val":
            thresholds.append(
                max(target_bbox[3] - target_bbox[1], target_bbox[2] - target_bbox[0]) * trg_scale
            )
        elif split == "trn":
            thresholds.append(
                max(source_bbox[3] - source_bbox[1], source_bbox[2] - source_bbox[0]) * src_scale
            )
            thresholds.append(
                max(target_bbox[3] - target_bbox[1], target_bbox[2] - target_bbox[0]) * trg_scale
            )
...
def load_pascal_data(path="data/PF-dataset-PASCAL", size=256, category="cat", split="test", subsample=None):
    ...
    for i in range(len(src_img_names)):
        ...
        if not split.startswith("train"):
            ...
            src_anns = os.path.join(path, "Annotations", category, os.path.basename(src_fn))[:-4] + ".mat"
            trg_anns = os.path.join(path, "Annotations", category, os.path.basename(trg_fn))[:-4] + ".mat"
