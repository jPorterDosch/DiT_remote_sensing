...
class LoRALinear(torch.nn.Module):
    def __init__(
        self, base: torch.nn.Linear, rank: int, alpha: float, dropout: float = 0.0
    ):
        ...
        self.B = torch.nn.Parameter(
            torch.empty(self.out_features, rank, device=device, dtype=dtype)
        )
...
def lora_wrap_flux(
    ...
    for i, block in enumerate(model.double_blocks):
        ...
        if not hasattr(block.img_attn, "qkv") or not hasattr(block.img_attn, "proj"):
            raise ValueError(
                f"double_blocks[{i}].img_attn missing qkv/proj: {type(block.img_attn)}"
            )
        ...
        if i in block_indices:
            ...
            if wrap_o:
                block.img_attn.proj = LoRALinear(
                    block.img_attn.proj, rank, alpha, dropout
                )
    ...
    for i, block in enumerate(model.single_blocks):
        if not hasattr(block, "linear1") or not hasattr(block, "linear2"):
            raise ValueError(
                f"Expected block to have linear1 and linear2 attributes, but got {type(block)}"
            )
